...
class DeepNN():
    ...
    def modify_array(self, array, indices_to_zero):
        """
        将指定的索引列下的元素修改为0。

        :param array: 输入的 M×N 维 numpy 数组
        :param indices_to_zero: 包含需要修改为0的列索引的列表
        """
        # 确保输入是 numpy 数组
    ...
    def feature_extract(self,state_batch):
        ...
        #将不同基下的测量结果提取出来
        m = int(len(state_batch[0])/3)

        indeces = []
        for i in range(3):
            indeces.append([i+3*x for x in range(m)])
        # 提取X测量基下的结果
        feature1 = np.copy(state_batch)
        feature1 = self.modify_array(feature1,indeces[0])
        # 提取Y测量基下的结果
        feature2 = np.copy(state_batch)
        feature2 = self.modify_array(feature2, indeces[1])
        # 提取Z测量基下的结果
        feature3 = np.copy(state_batch)
        feature3 = self.modify_array(feature3, indeces[2])
